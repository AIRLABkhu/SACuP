...
def pred_colorization(imgs, gt, mask, epoch, val_idx):
    ...
    obj_gt_list = [99999999]*12
    obj_mask_list = [99999999]*12
    for i in range(0, len(color_map)):
        ...
        if int(i) != 0:
            if len(idx_gt[0])!=0:
                obj_gt_list[i] = idx_gt[0][0]
            if len(idx_mask[0])!=0:
                obj_mask_list[i] = idx_mask[0][0]
    ...
    img_gt = imgs * 0.7 + gt_color * 0.3
    img_hstack = np.hstack((gt_color, mask_color, img_overlap, img_gt, img_pred))
    ...
    for i, obj in enumerate(CLASSES):
        if obj_gt_list[i]!=99999999:
            h, w = divmod(int(obj_gt_list[i]), 320)
            cv2.putText(img_hstack, obj, (w, h), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1,
                        cv2.LINE_AA)
            cv2.putText(img_hstack, obj, (w+320*1, h), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1,
                        cv2.LINE_AA)
            cv2.putText(img_hstack, obj, (w+320*2, h), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1,
                        cv2.LINE_AA)
            cv2.putText(img_hstack, obj, (w+320*3, h), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1,
                        cv2.LINE_AA)
        if obj_mask_list[i]!=99999999:
            h, w = divmod(int(obj_mask_list[i]), 320)
            cv2.putText(img_hstack, obj, (w+320*4, h), cv2.FONT_HERSHEY_DUPLEX, 0.4, (255, 255, 255), 1,
                        cv2.LINE_AA)
